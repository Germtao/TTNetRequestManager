...
- (BOOL)_dbSaveWithKey:(NSString *)key value:(NSData *)value fileName:(NSString *)fileName extendedData:(NSData *)extendedData {
    ...
    // 当filename为空的时候, 存在数据库的是value.bytes, 不然存的是NULL对象
...
- (BOOL)saveItemWithKey:(NSString *)key value:(NSData *)value filename:(NSString *)filename extendedData:(NSData *)extendedData {
    ...
    // _type为YYKVStorageTypeFile时候filename不能为空
...
/**
 *  YYKVStorage的type当为YYKVStorageTypeFile类型是根据key将本地和数据库都删掉,
 *  而YYKVStorageTypeSQLite是根据key删除掉数据库就好了
 */
 ...
 *  1. 因为在插入的时候我们就说了，当为YYKVStorageTypeFile类型的时候数据是存在本地文件的其他存在数据库。
 *     所以YYKVStorage对象先根据key从数据库拿到数据然后包装成YYKVStorageItem对象，然后再根据filename读取
 *     本地文件数据赋给YYKVStorageItem对象的value属性。
...
- (YYKVStorageItem *)getItemForKey:(NSString *)key {
    ...
    /**
     * 先从数据库读包装item 
     * 当时filename不为空的时候, 以为这数据库中没有存value值, 还得去文件中读出来value值 
     * 当时filename为空时, 意味着直接从数据库来拿取value值
     */
