...
/**
 *  在调用 YYKVStorage 对象的储存操作前主要做了下面几项操作:
 *  1. key和object的判空容错机制
 *  2. 利用runtime机制去取extendedData数据
 *  3. 根据是否定义了 _customArchiveBlock 来判断选择序列化object还是block回调得到value
 *  4. value的判空容错机制
 *  5. 根据 YYKVStorage 的type判断以及_inlineThreshold和value值的长度来判断是否选择以文件的形式储存value值。当value比较大时文件储存速度比较快。
 *  6. 如果_customFileNameBlock为空, 则根据key通过MD5加密得到转化后的filename. 不然直接拿到_customFileNameBlock关联的filename. 生成以后操作文件的路径
 *     做完上面的操作则直接调用 YYKVStorage 储存方法
 */
- (void)setObject:(id<NSCoding>)object forKey:(NSString *)key {
    ...
    // runtime 取 extended_data_key 的 value
    ...
    if (_kv.type != YYKVStorageTypeSQLite) {
        // 长度判断这个储存方式, value.length当大于_inlineThreshold, 则文件储存
...
/** 
 *  删除操作就是直接调用的 YYKVStorage 对象来操作了
 */
 ...
 *  最后再带一笔食物端最顶端的男人YYCache，当他写入的时候会同时调用YYDiskCache磁盘操作和YYMemoryCache内存操作。
 *  读取的时候先从内存读取，因为在内存的读取速度比磁盘快很多，如果没有读取到数据才会去磁盘读取。
 */
